...
export default function App() {
  ...
  // --- BACKGROUND MUSIC SYNC AND FIRST GESTURE AUTOPLAY SYSTEM ---
  useEffect(() => {
    // Attempt playback immediately when state changes
    if (isSongPlaying) {
      SoundCore.playBackgroundSong();
    } else {
      SoundCore.pauseBackgroundSong();
    }
  }, [isSongPlaying]);
  ...
  useEffect(() => {
    const handleGesture = () => {
      ...
      // Clean up after first user gesture
  ...
  return (
    <div className="min-h-screen bg-[#04060b] text-slate-100 font-sans antialiased overflow-x-hidden selection:bg-cyan-500 selection:text-slate-950 relative">
      ...
      {/* FLOATING PREMIUM HOVER MUSIC CONTROLLER WIDGET */}
      <div className="fixed top-4 right-4 z-[99] font-mono flex items-center gap-2">
        <motion.div 
          initial={{ opacity: 0, scale: 0.9, x: 20 }}
          animate={{ opacity: 1, scale: 1, x: 0 }}
          className="bg-[#070b13]/95 border border-amber-500/35 rounded-2xl p-2 pl-3 flex items-center gap-2.5 shadow-[0_0_25px_rgba(245,158,11,0.22)] backdrop-blur-md hover:border-amber-400 transition-all duration-300"
        >
          {/* Equalizer Waveforms */}
          <div className="flex items-end gap-[2px] h-3 px-0.5 select-none" title={isSongPlaying ? "Soundtrack Active" : "Soundtrack Paused"}>
            <div 
              className="w-[2px] bg-amber-400 rounded-full" 
              style={{ 
                height: "100%", 
                animation: isSongPlaying ? "equalizer 0.8s ease-in-out infinite" : "none",
            ...
            <div 
              ...
              style={{ 
                height: "100%", 
                animation: isSongPlaying ? "equalizer 0.5s ease-in-out infinite" : "none",
            ...
            <div 
              ...
              style={{ 
                height: "100%", 
                animation: isSongPlaying ? "equalizer 0.65s ease-in-out infinite" : "none",
                animationDelay: "0.3s",
                transformOrigin: "bottom",
                minHeight: "3.5px"
              }} 
          ...
          <div className="text-right select-none">
            <div className="text-[7px] text-slate-500 font-extrabold uppercase tracking-widest">AUDIO TRACK</div>
            <div className="text-[8px] text-amber-400 font-black tracking-wider uppercase">
              {isSongPlaying ? "ACTIVE_STREAM" : "MUTED"}
            </div>
          </div>
          ...
          <button
            onClick={() => {
              ...
              triggerToast(!isSongPlaying ? "🎵 BACKGROUND SONG STARTED" : "🎵 BACKGROUND SONG PAUSED", "success");
            }}
            className={`w-7.5 h-7.5 rounded-xl flex items-center justify-center border transition-all duration-300 cursor-pointer ${
              isSongPlaying 
                ? "bg-amber-500 text-slate-950 border-amber-300 shadow-[0_0_12px_rgba(245,158,11,0.4)] scale-105" 
                : "bg-slate-950 hover:bg-slate-900 text-slate-400 border-slate-900 hover:border-slate-800"
            }`}
            title={isSongPlaying ? "Pause Background Music" : "Play Background Music"}
          >
            {isSongPlaying ? (
              <Pause className="w-3.5 h-3.5 fill-current text-slate-950" />
            ) : (
              <Play className="w-3.5 h-3.5 translate-x-[0.5px] fill-current text-amber-500" />
            )}
          </button>
        </motion.div>
      </div>
        ...
        {!showSplash && isLoggedIn && (
          ...
          >
            ...
            <header className="mb-8 bg-[#070b13]/85 border border-[#22d3ee]/15 px-6 py-4 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-6 shadow-[0_0_20px_rgba(6,182,212,0.05)] backdrop-blur-md">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-end gap-2 sm:gap-3 w-full md:w-auto border-t md:border-t-0 border-slate-900/40 pt-4 md:pt-0 font-mono">
                <div className="flex items-center gap-1.5 bg-[#070b13]/85 border border-[#22d3ee]/10 px-2.5 py-1.5 rounded-xl text-[10px] sm:text-xs font-bold tracking-widest text-cyan-400 shadow-inner">
                  <Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-500 animate-spin" style={{ animationDuration: "16s" }} />
                  <span>{terminalClock}</span>
                </div>

                <div className="px-2.5 py-1.5 bg-slate-950 border border-slate-900 rounded-xl flex items-center gap-1.5">
                  <span className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${firebaseStatus === "ACTIVE" ? "bg-emerald-500 animate-pulse" : "bg-amber-500 animate-ping"}`} />
                ...
                <button
                  onClick={() => {
                    const active = !isSongPlaying;
                    setIsSongPlaying(active);
                    triggerToast(active ? "🎵 SONG PLAYBACK STARTED" : "🎵 SONG PLAYBACK STOPPED", "success");
                  }}
                  className={`px-2.5 py-1.5 rounded-xl text-[8px] sm:text-[9px] font-mono font-black tracking-widest transition uppercase flex items-center gap-1 sm:gap-1.5 cursor-pointer border ${
                    isSongPlaying 
                ...
                >
                  <Music className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${isSongPlaying ? "animate-bounce text-slate-950" : "text-amber-500"}`} />
                  <span>{isSongPlaying ? "SONG_ON" : "SONG_OFF"}</span>
                </button>

                <button
                  onClick={toggleMute}
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-slate-950 border border-slate-900 hover:border-cyan-400 text-cyan-400 flex items-center justify-center transition shrink-0"
                ...
                <button
                  ...
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-slate-950 border border-slate-900 hover:border-cyan-400 text-cyan-400 flex items-center justify-center transition relative group shrink-0"
                ...
                >
                  ...
                  >
                    <Bell className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyan-400 group-hover:text-cyan-300" />
                  </motion.div>
                  {notifications.filter(n => !n.read).length > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 flex h-4 w-4">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-4 w-4 bg-gradient-to-r from-rose-500 to-pink-600 border border-slate-950 text-[7px] font-mono font-black text-white items-center justify-center shadow-[0_0_8px_rgba(244,63,94,0.6)]">
                ...
                <button
                  ...
                  className="px-3 sm:px-4 py-1.5 sm:py-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-mono text-[9px] sm:text-[10px] font-black tracking-widest rounded-xl transition uppercase flex items-center gap-1 sm:gap-1.5 shadow-[0_0_15px_rgba(6,182,212,0.25)] shrink-0"
                >
                  <Settings className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-spin" style={{ animationDuration: "12s" }} />
                  <span>MENU</span>
                </button>
              </div>
            </header>
      ...
      <AnimatePresence>
        {(showSideDrawer || showProfileDrawer || showAdminDrawer || showNotificationsDrawer) && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.7 }}
            exit={{ opacity: 0 }}
