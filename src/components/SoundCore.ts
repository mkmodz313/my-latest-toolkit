...
class SoundCoreClass {
  ...
  constructor() {
    ...
    try {
      this.clickAudio = new Audio("/src/assets/click.mp3");
    ...
    try {
      this.syncAudio = new Audio("/src/assets/sync.mp3");
    ...
    try {
      this.songAudio = new Audio("/src/assets/song.mp3");
    ...
    try {
      ...
      if (this.isSongPlaying) {
        this.songAudio.play().catch((err) => {
          ...
          // Fallback to active click feedback or status change
        });
      } else {
